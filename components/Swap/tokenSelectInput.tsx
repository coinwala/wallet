...
export function Combobox({
  ...
  // Remove the useEffect that was previously setting the default token
  // This allows the component to update the selected token dynamically

  const handleSelect = (currentValue: string) => {
    setValue(currentValue);
    setOpen(false);

    if (onTokenSelect && tokens) {
      const selectedToken = tokens.find(
        (token) => token.symbol === currentValue
      );
